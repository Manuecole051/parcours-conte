...
window.topazeMgr = {
	...
	onLoad : function(){
		...
		try{
			...
			if (this.fNodeType == "nav" || this.fNodeType == "case") this.fNxtNodes[0] = scPaLib.findNodes(this.fPathAreaNodes,this.fMapRoot);
				...
				const vNodeArcs = scPaLib.findNodes("des:.arcBk", this.fArcRoot);
				for (i = 0; i < vNodeArcs.length; i++) {
					const vNodeArc = vNodeArcs[i];
			...
			// Manage cond nodes
			const vCondNodes = scPaLib.findNodes(this.fPathCondNodes);
			for (i = 0; i<vCondNodes.length; i++){
				const vCondNode = vCondNodes[i];
		...
		} catch(e){console.error("ERROR topazeMgr.onLoad: "+e)}
	},
	...
	initArcChoose : function(){
		let vDisplayNxt;
		for (let lvl in this.fNxtCnds) {
			const vNxtCnds = this.fNxtCnds[lvl];
			if (vNxtCnds.length>0){
				let vNxtFound = false;
				for (let i = 0; i < vNxtCnds.length; i++) {
					const vCnds = vNxtCnds[i];
					if(vCnds.cd) { // this is a standard cond
						for (let j = 0; j < vCnds.cd.length; j++) {
							const vCnd = vCnds.cd[j];
							vDisplayNxt = this.runCond(vCnd);
	...
	runCond : function(pCnd){
		...
		let vPl = null;
		let vOp;
		const vIsNode = pCnd.pl.tp == "n";
		const vIsVariable = pCnd.pl.tp == "uv" || pCnd.pl.tp == "av";
		const vVl = !vIsVariable ? this.xToFloat(pCnd.vl) : pCnd.vl;
		...
		switch (vOp){
			...
			case "=": {
				if (vIsVariable) {
					vPl = typeof vPl == "string" ? vPl.toLowerCase() : vPl;
	 ...
	 * @param pDec : number decimal places tu return
	 * @returns index value */
	getIndexVal : function(pId,pType,pDefaultVal, pDec){
		let vVl, vIdx, i;
		switch (pType){
			case "a": {
			...
			default: {
				...
				const vScore = scServices.assmntMgr.getScorePts(pId, this.fSessKey);
				vVl = !isNaN(parseFloat(vScore)) ? vScore : (pDefaultVal !== undefined ? pDefaultVal : (this.isNodeVisited(pId) ? 1 : 0));
				return pType == "u" ? this.xToFloat(vVl, pDec) : vVl; // if type = u (explicit) then guarantee float.
			}
		}
	},
	...
	getCalcInputFormula : function(pVl, pCalc){
		for (let i = 0; i < pCalc.length; i++) {
			...
			switch (vCalc.op){
				...
				case "rd": { //round
					const vDiv = Math.pow(10, vCalc.dm);
	...
	getScore : function(pNodeId, pSessKey, pFalseNumeric){
		...
		const vMin = scServices.assmntMgr.getMinPts(pNodeId, pSessKey);
		const vScore = scServices.assmntMgr.getScorePts(pNodeId, pSessKey);
		const vMax = scServices.assmntMgr.getMaxPts(pNodeId, pSessKey);
	...
	getScoreMax : function(pNodeId, pSessKey){
		...
		const vMin = scServices.assmntMgr.getMinPts(pNodeId, pSessKey);
		let vScore = scServices.assmntMgr.getResponse(pNodeId, pSessKey, "maxScorePts");
	...
	getScoreMin : function(pNodeId, pSessKey){
		...
		if (!this.isNodeFinished(pNodeId, pSessKey)) return null;
		const vMin = scServices.assmntMgr.getMinPts(pNodeId, pSessKey);
		let vScore = scServices.assmntMgr.getResponse(pNodeId, pSessKey, "minScorePts");
		if (typeof vScore == "undefined") vScore = scServices.assmntMgr.getScorePts(pNodeId, pSessKey);
		const vMax = scServices.assmntMgr.getMaxPts(pNodeId, pSessKey);
		return (vMax==vMin ? (vScore<=0 ? 0 : 100) : (vScore-vMin) / (vMax-vMin) * 100);
	},
	 ...
	 * @param pX : string input
	 * @param pDec : number of decimals to return (optional)
	 * @returns a float representation or 0 if input is NaN. */
	xToFloat : function(pX, pDec){
		let vY = parseFloat(pX);
		vY =  isNaN(vY)? 0 : vY;
		return (typeof pDec == "number" ? parseFloat(vY.toFixed(pDec)) : vY);
	},
	...
	xHasEmptyRequiredIndex : function(pNodes,pArc){
		...
		const vRequiredIndex = scPaLib.findNode("des:.required_index");
		if (a==pNodes.length) {

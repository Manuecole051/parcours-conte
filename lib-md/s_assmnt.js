...
scServices.suspendDataStorage = scOnLoads[scOnLoads.length] = scOnPageHides[scOnPageHides.length] = {
	_SendDataOnSave : true,

	getVal: function(pFields) {
		if(scServices.distribRecords && scServices.distribRecords.isDistribRecordsActive())
			return scServices.distribRecords.getField("suspendData",pFields)
		else{
	...
	setVal : function (pFields, pVal) {
		if(scServices.distribRecords && scServices.distribRecords.isDistribRecordsActive())
			return scServices.distribRecords.setField("suspendData",pFields, pVal)
		else{
			var vCur = this._Fields;
	...
	removeVal : function (pFields) {
		if(scServices.distribRecords && scServices.distribRecords.isDistribRecordsActive())
			return scServices.distribRecords.removeField("suspendData", pFields)
		else{
			var vCur = this._Fields;
			var imax = pFields.length-1;
			for(var i=0; i<imax; i++) {
	...
	commit : function(pForceSendData) {
		...
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
			...
			if(this._Dirty) vApi.SetValue("cmi.suspend_data", scServices.dataUtil.serialiseObjJs(this._Fields));
			...
			if(this._Dirty) vApi.LMSSetValue("cmi.suspend_data", scServices.dataUtil.serialiseObjJs(this._Fields));
			if(vMustSendData) vApi.LMSCommit("");
			else this._Uncommited = true;
		} else if(scServices.distribRecords && scServices.distribRecords.isDistribRecordsActive()){
			scServices.distribRecords.synch("suspendData");
		} else if(scServices.storage && scServices.storage.isStorageActive()) {
			if(this._Dirty) scServices.storage.getStorage().setItem(scServices.storage.getRootKey()+"suspend_data", scServices.dataUtil.serialiseObjJs(this._Fields));
		}
		this._Dirty = false;
	...
	pageHideSortKey: "3suspendDataStorage"
};
scServices.locationStorage = scOnLoads[scOnLoads.length] = {
	...
	getLocation : function(){
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
			...
			return scServices.distribRecords.getLocation();
		} else if(scServices.storage && scServices.storage.isStorageActive()) {
	...
	setLocation : function(pLoc, pForceSendData){
		...
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
			...
			scServices.distribRecords.setLocation(pLoc,vMustSendData);
		} else if(scServices.storage && scServices.storage.isStorageActive()) {
...
scServices.completionStorage = scOnLoads[scOnLoads.length] = scOnPageHides[scOnPageHides.length] = {
	_SendDataOnSave : true,
	...
	getSavedCompletionStatus : function(){
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
			...
			return scServices.distribRecords.getCompletionStatus();
		}
	},
	...
	*/
	setCompletionStatus : function(pStatus, pForceSendData){
		if(!pStatus || (this._LastStatusSet && this._LastStatusSet == pStatus)) return false;
		...
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
			...
			scServices.distribRecords.setCompletionStatus(pStatus,vMustSendData);
		}
		if(vMustSendData) this._Uncommited = false;
		return true;
	},
	...
	*/
	setProgression : function(pProgression, pForceSendData){
		if(!pProgression || (this._LastProgressionSet && this._LastProgressionSet == pProgression)) return false;
		this._LastProgressionSet = pProgression;
		var vMustSendData = pForceSendData!=undefined ?  pForceSendData : false;
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
			var vApi = scServices.scorm2k4.getScorm2k4API();
			vApi.SetValue("cmi.progress_measure", Math.round(pProgression*100)/100);
			this._Uncommited = true;
		}else if(scServices.scorm12 && scServices.scorm12.isScorm12Active()) {
			// non disponible
		} else if(scServices.distribRecords && scServices.distribRecords.isDistribRecordsActive()){
			scServices.distribRecords.setProgression(Math.round(pProgression*100)/100, vMustSendData);
		}
		return true;
	},
	...
	*/
	commit : function(pForceSendData){
		...
		if(vMustSendData && vIsDirty){
			if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
				...
				if(vMustSendData) scServices.distribRecords.synch("assmnt");
			}
		}
		if(vMustSendData) this._Uncommited = false;
	},
	...
	pageHideSortKey: "2completionStorage"
};

scServices.exitModeStorage = scOnLoads[scOnLoads.length] = scOnPageHides[scOnPageHides.length] = {
	...
	commitSessionTime : function(pForceSendData){
		...
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
			...
			scServices.distribRecords.setSessionTime(new Date().getTime() - this._timeStart,vMustSendData);
		}
		if(vMustSendData) this._Uncommited = false;
	},
	...
	terminate : function(pExitMode, pForceSendData){
		...
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
			...
			scServices.distribRecords.setExitStatus(pExitMode,vMustSendData);
	...
	pageHideSortKey: "3exitModeStorage"
};

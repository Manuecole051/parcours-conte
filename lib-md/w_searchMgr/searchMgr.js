...
window.searchMgr = {
	...
	fResultsAlwaysAccessible : false,
	...
	init: function(){
		try {
			...
			console.error("searchMgr.init::Error: "+e);
		}
	},
	...
	onLoad: function(){
		try {
			...
			this.fSearchLaunch = scDynUiMgr.addElement("input", this.fSearchCmds, "schBtnLaunch");
			...
			this.fResultFrame = this.xAddElt("div", scDynUiMgr.addElement("div", this.fRoot, "schResults"),"schResFrame");
			...
			const vHighLightBox = this.xAddElt("div", this.fResultFrame, "schHitBox");
			...
			console.error("searchMgr.onLoad::Error: "+e);
		}
	},
	...
	xUpdateUi: function(){
		...
		const vRes = scServices.scSearch.getLastResults(this.fIdxUrl);
		if (!vRes) return;
		...
		if (vRes && vRes.length > 0){
			...
			for (let i = 0; i < vRes.length; i++){
				const vResult = vRes[i];
				const vPageInfo = this.fPageList[vResult.url];
				const vPgeBk = this.xAddElt("div", this.fResultScroll, "schPgeBk schPgeRank_" + vResult.cat);
				const vPgeBtn = this.xAddBtn(vPgeBk, "schPgeBtn schPgeType_" + vPageInfo.type, vPageInfo.title);
				vPgeBtn.fUrl = scServices.scLoad.getRootUrl() + "/" + vResult.url;
				if (this.fResultsAlwaysAccessible || tplMgr.fHistMgr.isInHistory(vPgeBtn.fUrl)) vPgeBtn.href = vPgeBtn.fUrl;
				else {
					vPgeBtn.href = "#";
	...
	xHighlight: function(pRoot, pStr){
		const vTextNodes = [];
		const vNoIdxFilter = scPaLib.compileFilter(".noIndex|.footnotes|script|noscript");
